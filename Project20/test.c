...
//	//是地址，大小就是4/8个字节
//	printf("%d\n", sizeof(*a));//4
//	//*a中的a是数组首元素的地址a就是对首元素的地址解引用，找到的就是首元素
//	//首元素的大小就是4个字节
//	printf("%d\n", sizeof(a+1));//4
//	//这里的a是数组首元素的地址
//	//a+1是第二个元素的地址
...
//	//&a取出的是数组的地址
//	//&a-->int（*）[4]
//	//&a+1是从数组a的地址向后跳过了一个（4个整形元素的）数组的大小
//	//&a+1是地址，是地址就是4/8个字节
//	printf("%d\n", sizeof(&a[0]));//4
//	//&a[0]就是的一个元素的地址，计算的是地址的大小
//	printf("%d\n", sizeof(&a[0]+1));//4
//	//&a[0]+1是第二个元素的地址，计算的是地址的大小,大小是4/8个字节
//
//	return 0;
//}
...
//{
//	char arr[] = "abcdef";
//	//str1en是求字符串长度的，关注的是字符串中的\0，计算的是\0之前出现的字符的个数
//	//strlen是库函数，只针对字符串
//	//strlen只关注占用内存空间的大小，不在乎内存中放的是什么
//	//sizeof是操作符
//
//	
//	printf("%d\n", strlen(arr));//6
